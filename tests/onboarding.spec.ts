...
test("User sign-up and onboarding journey", async ({ page }) => {
  const randomEmail = `test-${Math.random().toString(36).substring(2, 9)}@stanford.edu`;
  ...
  await page.fill('input[placeholder="alexchen"]', randomUsername);
  await page.fill('input[placeholder="Palo Alto, CA"]', "Stanford, CA");
  ...
  await page.fill('input[placeholder="Stanford University"]', "Stanford University");
